#!/bin/bash
LIBRARY_NAME=$(grep -m 1 name library/pyproject.toml | awk -F" = " '{print substr($2,2,length($2)-2)}')
DOCS_LIBRARY_NAME="plasma"
...
CONFIG_VARS=$(
	$PYTHON - <<EOF
import toml
config = toml.load("library/pyproject.toml")
...
if confirm "Would you like to generate documentation?"; then
	...
	if $PYTHON -m pdoc "$DOCS_LIBRARY_NAME" -o "$RESOURCES_DIR/docs" > /dev/null; then
